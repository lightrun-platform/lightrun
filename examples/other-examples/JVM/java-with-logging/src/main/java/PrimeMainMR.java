...
public class PrimeMainMR {
    private static Logger logger = LoggerFactory.getLogger(PrimeMainMR.class.getName());
    ...
    public static void main(String[] args) throws InterruptedException {
        for (int i = 2; i < Math.pow(10, 9); ++i) {
            if (i % 1000 == 0) {
            logger.info("We have counted " + i + " numbers so far, out of which only " + cnt + " are prime");
            }
            if (isPrime(i)) {
                cnt++;
            }
